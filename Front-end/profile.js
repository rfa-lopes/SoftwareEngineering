      ...
      if (response) {
        ...
        for (var k = 0; k < response.length; k++) {
          p +=
            ...
            response[k].id +
            '" class="product__item__pic set-bg" data-setbg=data:image/png;base64,"' +
            response[k].image +
            'data-toggle="modal" data-target="#postModal" onclick="save(' +
            response[k].id +
            ')"></div><div class="product__item__text"><h6><a href="#">' +
            response[k].description +
            "</a></h6></div></div></div>";
        }
        document.getElementById("mypostsdiv").innerHTML = p;
        for (var j = 0; j < response.length; j++) {
          document.getElementById(response[j].id).style.backgroundImage =
            "url('data:image/png;base64," + response[j].image + "')";
        }
      } else {
        alert("No response");
      }
        ...
        for (var k = 0; k < response.length; k++) {
          if (k == 0) {
            m +=
              ...
              response[k].image +
              '"/></div><div class="product__discount__item__text"><h4><a>' +
              username +
              "</a></h4></div></div></div>";
          } else {
            m +=
      ...
      if (response) {
        for (var j = 0; j < response.length; j++) {
          switch (response[j].type) {
            ...
            case 2:
              var c3 = parseInt(
                document.getElementById("laughc").innerHTML,
                10
              );
              c3 += 1;
              document.getElementById("laughc").innerHTML = c3;
            ...
            case 4:
              var c5 = parseInt(
                document.getElementById("angryc").innerHTML,
                10
              );
              c5 += 1;
              document.getElementById("angryc").innerHTML = c5;
