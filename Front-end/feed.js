    ...
    success: function (response) {
      if (response) {
        ...
        for (var k = 0; k < response.length; k++) {
          m +=
            '<div class="col-lg-4"><div class="product__discount__item"><div id="' +
            response[k].id +
            '" class="product__discount__item__pic set-bg" data-setbg="data:image/png;base64,' +
            response[k].image +
            '"><ul class="product__item__pic__hover"><li><a href="#sharestorymodal" data-toggle="modal" onclick="save(' +
            response[k].id +
            ')"><i class="fa fa-retweet"></i></a></li><li><a href="#postModal" data-toggle="modal"><i class="fa fa-envelope"></i></a></li></ul></div><div class="product__discount__item__text"><h5><a>' +
            username +
            "</a></h5></div></div></div>";
        }
        document.getElementById("storyfeed").innerHTML = m;
        for (var t = 0; t < response.length; t++) {
          document.getElementById(response[t].id).style.backgroundImage =
            "url('data:image/png;base64," + response[t].image + "')";
        }
      } else {
        alert("No response");
      }
      ...
      if (response) {
        ...
        for (var i = 0; i < response.length; i++) {
          ...
          n +=
            ...
            response[i].id +
            ')"><i class="fa fa-retweet"></i></a></li><li><a href="#"><i class="fa fa-plus"></i></a></li></ul></div><div class="product__item__text"><h6><a href="#">' +
            response[i].description +
            "</a></h6><h5>" +
            username +
            "</h5></div></div></div>";
